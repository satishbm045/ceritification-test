<template>
    <div class="qn-layout">
        ...
        <div class="right-side" v-if="questionSetData.details.length > 0">
            <div class="qn-no" v-for="(eachQuestionData,index) in questionSetData.details" :key="index">
                <div class="number" :class="{'done' : eachQuestionData.isAnswered, 'active': isActive(index)}" v-on:click="shortClick(index)">
                    {{index+1}}
                </div>
            </div>
