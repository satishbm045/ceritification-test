    ...
    <div class="inputBox">
      ...
      <div class="right">
        <select
          ...
          v-on:change="filterLanguage(selectedLanguage)"
        >
          <option disabled selected>Select</option>
          <option v-for="(language, index) in questionSet.data" :key="index">{{
            language.type
          }}</option>
        </select>
      </div>
...
export default {
  ...
  methods: {
    filterLanguage(language) {
      this.filteredQuestionSet = this.questionSet.data.filter(
        eachSet => eachSet.type.toLowerCase() == language.toLowerCase()
      );
    },
    submitBtn() {
      ...
      this.$store.commit("updateOfNameAndLanguage", {
        selectedLanguage: this.selectedLanguage,
        nameOfCandidate: this.nameOfCandidate
      });
